...
conn.on('ready', () => {
  ...
  const localServiceFile = path.join(__dirname, 'bot', 'volki-bot.service');
  const remoteServiceFile = '/etc/systemd/system/volki-bot.service';
  ...
  conn.sftp((err, sftp) => {
    ...
    sftp.fastPut(localBotFile, remoteBotFile, (uploadErr) => {
      if (uploadErr) {
        console.error('Upload error (bot):', uploadErr);
        conn.end();
        process.exit(1);
      }
      console.log('Uploaded volki_bot.py successfully!');
      
      sftp.fastPut(localServiceFile, remoteServiceFile, (srvErr) => {
        if (srvErr) {
          console.error('Upload error (service):', srvErr);
          conn.end();
          process.exit(1);
        }
        console.log('Uploaded volki-bot.service successfully!');
        
        // Reload systemd daemon and restart the service
        const reloadAndRestartCmd = 'systemctl daemon-reload && systemctl restart volki-bot';
        console.log(`Executing remote command: ${reloadAndRestartCmd}`);
        
        conn.exec(reloadAndRestartCmd, (execErr, stream) => {
          if (execErr) {
